...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Aktifkan logging agar semua debug/info/error dari librjss muncul
    ...
    // Tentukan auth mode: token lebih diutamakan jika tersedia
    let auth_mode = if let (Some(key), Some(secret)) = (api_key, api_secret) {
        ...
        eprintln!("Either JSS_EMAIL+JSS_PASSWORD or JSS_TOKEN_API_KEY+JSS_TOKEN_API_SECRET must be set");
        std::process::exit(1);
    };
    ...
    let max_retries: u32 = env::var("JSS_MAX_RETRIES")
        ...
        .unwrap_or(3);
    let user_agent =
        env::var("JSS_USER_AGENT").unwrap_or_else(|_| "cli-auth-test/1.0".into());
    ...
    match client.authenticate().await {
        ...
        Err(e) => {
            ...
            // Detail lengkap sudah muncul di log karena tracing diaktifkan.
            std::process::exit(1);
        }
    }
